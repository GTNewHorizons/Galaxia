...
class InterplanetaryTransferSystemTest {
    ...
    void transferJobIsCreated() {
        ...
        OrbitalCelestialBody source = root.children().get(0);
        OrbitalCelestialBody destination = root.children().get(1);
        ...
        assertTrue(!transfer.trajectory().isEmpty(), "Trajectory must not be empty");
    }
    ...
    void transferTrajectoryPointsAreFinite() {
        ...
        assertNotNull(transfer);
        for (TransferTrajectoryPoint pt : transfer.trajectory()) {
            assertTrue(Double.isFinite(pt.worldX()), "Non-finite worldX in trajectory");
            assertTrue(Double.isFinite(pt.worldY()), "Non-finite worldY in trajectory");
        }
    }
    ...
    void currentTransferPointInterpolatesCorrectly() {
        ...
        TransferTrajectoryPoint startPoint = InterplanetaryTransferSystem
            .getCurrentTransferPoint(transfer, transfer.departureTime());
        TransferTrajectoryPoint endPoint = InterplanetaryTransferSystem
            .getCurrentTransferPoint(transfer, transfer.arrivalTime());
    ...
    void lambertSolverReturnsValidVelocities() {
        ...
        double[] result = InterplanetaryTransferSystem.solveLambert(r1, 0, -r2, 0, tof, mu, true);
        assertNotNull(result, "Lambert solver should return a result for valid Hohmann transfer");
        for (double v : result) {
            assertTrue(Double.isFinite(v), "Lambert result must be finite, got " + v);
        }
    }
    ...
    void hemateriaToOrbitingPanspiraSpeedFactorTransfersAreCreated() {
        ...
        OrbitalCelestialBody hemateria = root.children().get(0);
        OrbitalCelestialBody panspira = root.children().get(1);
        InterplanetaryTransferSystem.OrbitalTransferSupport transferSupport =
            new InterplanetaryTransferSystem.OrbitalTransferSupport();
    ...
    private static void assertTrajectoryHasNoAbruptTurns(InterplanetaryTransferJob transfer, double speedFactor) {
        TransferTrajectoryPoint previousPoint = InterplanetaryTransferSystem
            .getCurrentTransferPoint(transfer, transfer.departureTime());
        ...
        for (int i = 1; i <= TRAJECTORY_SAMPLES; i++) {
            ...
            TransferTrajectoryPoint point = InterplanetaryTransferSystem.getCurrentTransferPoint(transfer, sampleTime);
            assertNotNull(point, "Missing sample point " + i + " for x=" + speedFactor);
            ...
            double dx = point.worldX() - previousPoint.worldX();
            double dy = point.worldY() - previousPoint.worldY();
        ...
        assertTrue(
            ...
            "Potential trajectory jump for x=" + speedFactor + ": max segment " + maxSegmentLength
                + " avg " + averageSegmentLength);
    }
