...
public class RocketSchematicItemRenderer implements IItemRenderer {
    ...
    private void renderSchematic(ItemStack stack, TextureManager textureManager) {
        ...
        RocketBlueprint blueprint =
            ItemRocketSchematic.readBlueprint(stack);
        ...
        for (RocketPartInstance part : parts) {
            ...
            ResourceLocation texture = part.def().textureLocation();
            ...
            float x = PADDING
                + (usable - totalWidth * pixelsPerCell) / 2f
            ...
            drawQuad(textureManager, texture, x, y, w, h, -0.02f);
        }

        GL11.glPopAttrib();
        GL11.glPopMatrix();

        drawTexts(assembly, blueprint, stack);
    }
    ...
    private static void drawTexts(
        ...
        String[] lines = {
            StatCollector.translateToLocalFormatted(
                "galaxia.item.schematic.label.name",
                blueprint.getName()
            ),
            StatCollector.translateToLocalFormatted(
                "galaxia.item.schematic.label.stages",
                assembly.getStageCount()
            ),
            StatCollector.translateToLocalFormatted(
            ...
            StatCollector.translateToLocalFormatted(
                "galaxia.item.schematic.label.delta_v",
                (int) assembly.getTotalDeltaV()
            ),
            StatCollector.translateToLocalFormatted(
                "galaxia.item.schematic.label.viable",
                assembly.viable() ? "YES" : "NO"
            )
        };
