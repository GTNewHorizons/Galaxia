...
final class OrbitalSceneRenderer {
    ...
    void drawBodies(OrbitalSceneFrame frame, OrbitalCelestialBody viewRoot) {
        for (ResolvedBodyDrawState state : frame.resolvedBodies) {
            if (state.body.objectClass() == CelestialObjectClass.GALAXY || state.bodyAlpha <= 0.01f || !state.renderBody) {
                continue;
            }
            ...
            if (texture != null && callbacks.getDisplaySpriteSize(state.body) > 0.0001f) {
                ...
                int color = getFallbackBodyColor(state.body.objectClass());
    ...
    void drawOrbits(OrbitalSceneFrame frame, float ellipseAlpha) {
        ...
        for (ResolvedBodyDrawState state : frame.resolvedBodies) {
            if (state.parent == null || !state.renderBody
                || OrbitalWorldStateCache.usesAbsolutePosition(state.parent, state.body)) {
                continue;
            }
            ...
            drawEllipse(state.body.orbitalParams(), parentState.worldX, parentState.worldY, ellipseAlpha);
        }
    }
