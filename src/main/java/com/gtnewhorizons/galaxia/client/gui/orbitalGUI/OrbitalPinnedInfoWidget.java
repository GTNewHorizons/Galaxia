...
final class OrbitalPinnedInfoWidget extends ParentWidget<OrbitalPinnedInfoWidget> {
    ...
    private void buildRow(ParentWidget<?> root, Minecraft mc, PinnedInfoRow row, int contentWidth, int y) {
        ...
        root.child(new TextWidget<>(row.label).color(0xFF5A63FF).shadow(true).pos(PANEL_PADDING, y));
        ...
        for (String line : wrappedLines) {
            root.child(new TextWidget<>(line).color(0xFFD9E0FF).shadow(true).pos(PANEL_PADDING, lineY));
    ...
    private void buildItemGrid(ParentWidget<?> root, List<ItemStack> items, int x, int y, int contentWidth) {
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            root.child(createItemWidget(stack, ICON_SIZE).pos(itemX, itemY).size(ICON_SIZE, ICON_SIZE));
        }
    }
    ...
    private void buildInlineRow(ParentWidget<?> root, Minecraft mc, PinnedInfoRow row, int contentWidth, int y) {
        ...
        root.child(new TextWidget<>(label).color(0xFFD9E0FF).shadow(true).pos(PANEL_PADDING, y + 1));
        ...
        for (int i = 0; i < row.items.size(); i++) {
            ...
            root.child(createItemWidget(stack, INLINE_ICON_SIZE).pos(itemX, y).size(INLINE_ICON_SIZE, INLINE_ICON_SIZE));
        }
    }
    ...
    private IDrawable createBackgroundDrawable() {
        return drawable((context, x, y, width, height) -> {
            Gui.drawRect(x, y, x + width, y + height, 0xFF162133);
        });
    }
