...
public class TierMatchesDestinationValidator implements IRocketValidator {
    ...
    public ValidationResult validate(RocketAssembly assembly) {
        ...
        EnumTiers tier = cores.get(0).getTier();
        String destinationName = SolarSystemRegistry.getById(destinationId).name();
        EnumTiers destinationTier = SolarSystemRegistry.getById(destinationId).tier();
        boolean ok = tier.isGreaterThanOrEqual(SolarSystemRegistry.getById(destinationId).tier());
        return ok ? ValidationResult.success()
                : new ValidationResult(false, String.format("Rocket of tier %d cannot reach %s (Tier %d)", tier.toInt(),
                        destinationName, destinationTier.toInt()));
    }
}
