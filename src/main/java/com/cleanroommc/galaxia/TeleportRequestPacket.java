    ...
    public static class Handler implements IMessageHandler<TeleportRequestPacket, IMessage> {
        ...
        public IMessage onMessage(TeleportRequestPacket message, MessageContext ctx) {
            ...
            if (player.dimension == message.dim) {
                player.setLocationAndAngles(message.x, message.y + 0.5, message.z, player.rotationYaw, player.rotationPitch);
            ...
            server.getConfigurationManager().transferPlayerToDimension(player, message.dim, new Teleporter(targetWorld) {

                @Override
                public void placeInPortal(Entity entity, double px, double py, double pz, float yaw) {
                    entity.setLocationAndAngles(message.x, message.y + 0.5, message.z, entity.rotationYaw, entity.rotationPitch);
