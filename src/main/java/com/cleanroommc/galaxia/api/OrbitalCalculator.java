...
public class OrbitalCalculator {
    ...
    public static double calculateEffectiveExhaustVelocity(SpaceBody launchBody, Rocket rocket, double launchAltitude) {
        return rocket.getPropellant()
            .getSpecificImpulse() * (launchBody.getMass() * gravitationalConstant)
            / Math.pow(launchAltitude, 2);
    }
     ...
     */
    public static double calculateDirectDeltaV(SpaceBody launchBody, SpaceBody targetBody, double launchAltitude,
    ...
    public static double calculateRequiredFuelVolume(SpaceBody launchBody, Rocket rocket, double deltaV,
        ...
        final double effectiveExhaustVelocity = calculateEffectiveExhaustVelocity(launchBody, rocket, launchAltitude);
        return rocket.getBaseMass() * (Math.exp(deltaV / effectiveExhaustVelocity) - 1);
    }

    public static boolean isEnoughFuelForTransfer(SpaceBody launchBody, Rocket rocket, double deltaV,
        double launchAltitude) {
        if (rocket.getFuelMass() == 0) {
            return false;
        }

        final double requiredVolume = calculateRequiredFuelVolume(launchBody, rocket, deltaV, launchAltitude);
        return rocket.getFuelMass() <= requiredVolume;
